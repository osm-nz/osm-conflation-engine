...
const config: ChangesetWatchConfig = {
  ...
  refTag: 'ref:linz:address_id',
  getLocalKey({ tags }) {
    return `${tags?.['addr:housenumber']} ${tags?.['addr:street']}`;
  },
  watchArea: [1, 2, 3, 4],
  getLabel: ({ tags }) => tags?.['addr:suburb'] || '',
};

describe(checkDiffsForAddress, () => {
  it('can figure out if any addresses were deleted', () => {
    expect(checkDiffsForAddress(config, [{ cs, diff }])).toStrictEqual([
      {
        refTag: 'ref:linz:address_id',
        rowId: '2015482',
        changeset: 123456,
        note: '(iD): update Eastern Busway construction',
        timestamp: '2021-12-01T00:11:34.000Z',
        local_key: '3 Marine Parade',
        label: 'Devonport',
        username: 'kylenz',
        operator: '',
