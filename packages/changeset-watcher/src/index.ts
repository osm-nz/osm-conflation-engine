...
import { DEFAULT_IS_IMPORTER } from './constants.js';
...
export async function run(config: ChangesetWatchConfig): Promise<void> {
  const { refTag, watchArea, isImportUser = DEFAULT_IS_IMPORTER } = config;
  const lastCheck = await getLastCheckDate(refTag);
  ...
  const addrIssues = checkDiffsForAddress(config, diffsWithDeletedTags);

  console.log('Total issues:', addrIssues.length);

  // save the results of these changesets
  await updateIgnoreList(config, addrIssues);

  // done
  await updateLastCheckDate(config);
}
