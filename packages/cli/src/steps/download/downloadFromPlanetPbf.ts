import { setTimeout } from 'node:timers/promises';
import { spawn } from 'node:child_process';
import { promises as fs } from 'node:fs';
import { basename, dirname } from 'node:path';
import { getHttpHeaders } from '../../constants/defaults.js';
import type { Ctx } from '../../types/index.js';

const DOWNLOAD_SERVER = 'https://osm-internal.download.geofabrik.de';
const DOWNLOAD_SERVER_NO_METADATA = 'https://download.geofabrik.de';
const OSM_SERVER = 'https://www.openstreetmap.org';

const getCsrfToken = (html: string) =>
  html.match(/name="csrf-token" content="([^"]+)"/)![1]!;

/**
 * based on these docs: https://github.com/geofabrik/sendfile_osm_oauth_protector/blob/master/doc/client.md
 * This basically scrapes the OSM homepage to login programmatically,
 * bypassing CSRF and bypassing OAuth consent...
 */
async function getAuthToken(ctx: Ctx, pbfUrl: string) {
  if (!pbfUrl.startsWith(DOWNLOAD_SERVER)) {
    ctx.warnings.push(
      `(!) Planet PBF url is not from ${DOWNLOAD_SERVER}, so metadata may not be available.`,
    );
  }

  if (!process.env.OSM_AUTH) {
    ctx.warnings.push(
      '(!) No OSM credentials, so the planet extract will not include metadata',
    );
    return {
      url: pbfUrl
        .replace(DOWNLOAD_SERVER, DOWNLOAD_SERVER_NO_METADATA)
        .replace('-internal.osm.pbf', '.osm.pbf'),
      cookies: '',
    };
  }

  const credentials = new URLSearchParams(process.env.OSM_AUTH);
  const baseHeaders = getHttpHeaders(ctx.config);

  // 1. fetch config
  const authConfig = (await fetch(
    `${DOWNLOAD_SERVER}/get_cookie?action=get_authorization_url`,
    { method: 'POST', headers: baseHeaders },
  ).then((r) => r.json())) as {
    authorization_url: string;
    state: string;
    client_id: string;
    redirect_uri: string;
  };

  // 2. fetch home page to get session cookies + CSRF token
  const homePage = await fetch(`${OSM_SERVER}/login?cookie_test=true`, {
    headers: baseHeaders,
  });
  let cookieJar = homePage.headers.get('Set-Cookie')!;

  // 3. login
  const loginPage = await fetch(`${OSM_SERVER}/login`, {
    method: 'POST',
    body: new URLSearchParams({
      username: credentials.get('username')!,
      password: credentials.get('password')!,
      referer: '/',
      commit: 'Login',
      authenticity_token: getCsrfToken(await homePage.text()),
    }),
    headers: {
      ...baseHeaders,
      Cookie: cookieJar,
      'Content-Type': 'application/x-www-form-urlencoded',
    },
  });
  if (loginPage.status !== 200) throw new Error('Login failed');
  cookieJar += `; ${loginPage.headers.get('Set-Cookie')!}`;

  // 4. open the OAuth consent page
  const consentPage = await fetch(authConfig.authorization_url, {
    headers: { ...baseHeaders, Cookie: cookieJar },
  });

  // 5. if there is no consent, try to submit the form
  const authToken = consentPage.url.startsWith(OSM_SERVER)
    ? await fetch(authConfig.authorization_url, {
        method: 'POST',
        body: new URLSearchParams({
          client_id: authConfig.client_id,
          redirect_uri: authConfig.redirect_uri,
          authenticity_token: getCsrfToken(await consentPage.text()),
          state: authConfig.state,
          response_type: 'code',
          scope: 'read_prefs',
          nonce: '',
          code_challenge: '',
          code_challenge_method: '',
          commit: 'Authorize',
        }),
        headers: {
          ...baseHeaders,
          Cookie: cookieJar,
          'Content-Type': 'application/x-www-form-urlencoded',
        },
      }).then((r) => r.text())
    : await consentPage.text();

  if (!authToken.includes('gf_download_oauth')) throw new Error('No token');

  return {
    url: pbfUrl,
    cookies: authToken,
  };
}

export async function downloadFromPlanetPbf(ctx: Ctx, pbfUrl: string) {
  const planetFile = ctx.tempFileNames.pbf;

  const { cookies, url } = await getAuthToken(ctx, pbfUrl);

  await fs.mkdir(dirname(planetFile), { recursive: true });
  await fs.rm(planetFile, { recursive: true, force: true });

  if (/^https?:\/\//.test(url)) {
    // it's an actuall URL, so fetch it
    console.info('downloading from', url, 'to', planetFile);
    const child = spawn(
      'curl',
      ['-L', url, '-H', `Cookie: ${cookies}`, '--output', basename(planetFile)],
      { cwd: dirname(planetFile) },
    );

    child.stdout.pipe(process.stdout);
    child.stderr.pipe(process.stderr);

    await new Promise<void>((resolve, reject) => {
      child.on('close', (code) => (code ? reject(code) : resolve()));
    });

    // ensure that the file is written to the disk properly
    await setTimeout(5000);
  } else {
    // else: it's a path to a file on disk, so just copy the file.
    // this is only used for e2e tests by consumers, so no harm
    // in copying/duplicating the file.
    console.info('copying file from', url, 'to', planetFile);
    await fs.copyFile(url, planetFile);
  }

  console.info(planetFile, await fs.stat(planetFile));
}
